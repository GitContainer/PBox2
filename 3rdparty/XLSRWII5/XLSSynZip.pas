...
{$endif}
    ...
    function UnZipStream(const aName: TFileName): TStream; overload;
    /// uncompress a file stored inside the .zip archive into a destination directory
...
{$endif Linux}
  ...
  public
    ...
    procedure AddStream(const aName: TFileName; aStream: TStream);
...
procedure TZipWriteAbstract.AddDeflated(const aZipName: TFileName; Buf: pointer; Size, CompressLevel, FileAge: integer);
var tmp: pointer;
    tmpsize: integer;
begin
  if self=nil then
  ...
  with Entry[Count] do begin
    with fhr.fileInfo do begin
      zcrc32 := XLSSynZip.crc32(0,Buf,Size);
      zfullSize := Size;
...
procedure TZipWriteAbstract.AddStream(const aName: TFileName; aStream: TStream);
var
  Buf: Pointer;
begin
  GetMem(Buf,aStream.Size);
  try
    aStream.Write(Buf^,aStream.Size);
    AddDeflated(aName,Buf,aStream.Size);
  finally
    FreeMem(Buf);
  end;
end;
...
begin
  ...
  if (len<>info.zfullsize) or
     (info.zcrc32<>XLSSynZip.crc32(0,pointer(result),info.zfullSize)) then
...
function TZipRead.UnZip(aIndex: integer; const DestDir: TFileName; DestDirIsFileName: boolean): boolean;
var FS: TFileStream;
    Path: TFileName;
    info: TFileInfo;
    CRC: Cardinal;
begin
  ...
  try
    ...
    Z_STORED: begin
      ...
      CRC := XLSSynZip.crc32(0,Entry[aIndex].data,info.zfullSize);
    end;
    Z_DEFLATED:
...
function TZipRead.UnZipStream(const aName: TFileName): TStream;
var info: TFileInfo;
    CRC: Cardinal;
    i: integer;
begin
  ...
  if not RetrieveFileInfo(i,info) then
    exit;

  case info.zZipMethod of
    Z_STORED: begin
      Result.Write(Entry[i].data^,info.zfullsize);
      CRC := XLSSynZip.crc32(0,Entry[i].data,info.zfullSize);
    end;
    Z_DEFLATED:
      if UnCompressStream(Entry[i].data,info.zzipsize,Result,@CRC)<>info.zfullsize then
        exit;
    else raise ESynZipException.CreateFmt('Unsupported method %d for %s',[info.zZipMethod,Entry[i].zipName]);
  end;

  Result.Seek(0,soFromBeginning);

  if CRC <> info.zcrc32 then begin
    Result.Free;
    Result := Nil;
  end;
end;
...
begin
  ...
  if (UnCompressMem(@gz[10],pointer(result),gzLen-18,Len)<>Len) or
     (XLSSynZip.crc32(0,pointer(result),Len)<>pCardinal(@gz[gzLen-8])^) then
    result := ''; // invalid CRC
end;
...
function CompressStream(src: pointer; srcLen: integer; aStream: TStream; CompressionLevel: integer=6; ZlibFormat: Boolean=false): cardinal;
var strm: TZStream;
    code: integer;
...
constructor TZipRead.Create(aStream: TStream);
var
  Buf: Pointer;
begin
  GetMem(Buf,aStream.Size);
