...
{$endif}
    ...
    property MissingBookmarks: TRawUTF8List read fMissingBookmarks;
  end;
...
{$endif}
  ...
  public
    ...
    property TM: TTextMetric read fM;
    property OTM: TOutlineTextmetric read fOTM;
  end;
...
//{$message warn '__DEBUG__'}
//      if TmpSize<1000000 then // don't compress tiny blocks
      if TmpSize<100 then // don't compress tiny blocks
        FFilter := ''
      else begin
...
begin
  if SysLocale.FarEast then begin
    ...
    result := XLSSynCommons.StrLen(pointer(Text));
end;
...
begin
  ...
  with ARect do begin
    ...
    FAnnotation.AddItem('BS',TPdfRawText.Create('<</W 0>>'))
  end;
...
begin
  ...
  if NeedFileID then begin
    ...
    P := pointer(IDs);
    P[0] := '<';
    XLSSynCommons.BinToHex(PAnsiChar(@FileID[0]),P+1,16);
    P[33] := '>';
    {$endif}
